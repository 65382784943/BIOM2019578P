...
# Main analysis only S2P and death / heart transplant

censorCR4   <- censorCR1 + censorCR3
censorCR5   <- censorCR0 + censorCR2

data        <- cbind(data, eventCR_time, censorCR4, censorCR5)

dataLong    <-
  dataLongCompRisks(
    dataSet      = data,
    timeColumn   = "eventCR_time",
    eventColumns = c('censorCR4', 'censorCR5')
  )
dataLong$eT <- dataLong$e1 + dataLong$e2
...
multLogitVGM <-
  vgam(
    cbind(e0, e1, e2) ~ timeInt + factor(GENDER) + factor(RACE2) + factor(ctrt) + BWT + reverse.levels(factor(PRENATDX)) +
...
avg_ft           <- cbind(apply(f_t1, 2, mean), apply(f_t2, 2, mean))
colnames(avg_ft) <- c('Death_HrtTrans', 'S2P')
...
legend(
  2.8,
  0.35,
  c('R=1: S2P',  'R=2: Death/Heart Transplant'),
  col = c('red', 'black'),
  lty = c(1, 4),
  cex = 1.6,
  lwd = 2.8
